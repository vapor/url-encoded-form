...
class URLEncodedFormParserTests: XCTestCase {
    func testBasic() throws {
        ...
        let form = try URLEncodedFormParser().parse(data: data)
    ...
    func testOptions() throws {
        let data = "hello=&foo"
        let normal = try URLEncodedFormParser().parse(data: data)
        let noEmpty = try URLEncodedFormParser(omitEmptyValues: true).parse(data: data)
        let noFlags = try URLEncodedFormParser(omitFlags: true).parse(data: data)
